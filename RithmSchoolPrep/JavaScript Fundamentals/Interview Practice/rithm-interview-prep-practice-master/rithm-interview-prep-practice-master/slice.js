...
function slice(arr, start, end) {
  // declare newOutput array
  let output = [];
  //  check if end is undefined or larger than arr length
  if (end === undefined || end > arr.length) {
    end = arr.length;
  }
  // iterate over array starting at start and continuing until end
  for (let i = start; i < end; i++) {
    // push element at i to output
    output.push(arr[i]);
  }
  // return output
  return output;
